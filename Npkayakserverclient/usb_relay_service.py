...
libpath = "/home/odroid/usb-relay-hid/_rel/bin-Linux-x86/"
...
def check_devices():
    ...
    for dev in devids:
      ...
      print("Testing relay with ID=" + dev)
      openDevById(dev)
      testR2()
      closeDev()
    '''

def startContinuousCycle(stop_event, deviceId, relayId, delay):
     while not stop_event.wait(0.01):
	   print datetime.datetime.now()
           lock.acquire()
           hdev, numch = openDevById(deviceId)
           print("Turn on %s relay %s" % (deviceId, relayId))
           ret = L.usb_relay_device_open_one_relay_channel(hdev,relayId)
           if ret != 0: fail("Failed openOneRelayChannel()!")
           closeDev(hdev)
	   lock.release()
           time.sleep(delay)
	   ...
	   lock.acquire()
	   hdev, numch = openDevById(deviceId)
	   print("Turn off %s relay %s" % (deviceId, relayId))
           ret = L.usb_relay_device_close_one_relay_channel(hdev,relayId)
           if ret != 0: fail("Failed closeOneRelayChannel()!")
           closeDev(hdev)
           lock.release()
           time.sleep(delay)

def startOneCycle(stop_event, deviceId, relayId, delay):
     ...
     threadName = "%s-%s" % (deviceId, relayId)
     if threadName in threads:
           print "Stopped ", threadName
           threads.pop(threadName)
...
class RpcService(object):
    ...
    def openOneRelayChannel(self, deviceId,relayId):
            lock.acquire()
            print("On one reply......")
            hdev, numch = openDevById(deviceId)
            print("Turn on %s relay %s" % (deviceId, relayId))
    ...
    def closeOneRelayChannel(self, deviceId,relayId):
        lock.acquire()
        print("Off one reply......")
        hdev, numch = openDevById(deviceId)
        print("Turn off %s relay %s" % (deviceId, relayId))
    ...
    def runContinuousCycle(self, deviceId, relayId, delay):
         threadName = "%s-%s" % (deviceId, relayId)
         if threadName in threads:
                print ("Duplicate continuous cycle for %s" % threadName)
                return

         print("Start continuous cycle")
         pill2kill = threading.Event()
         t = threading.Thread(target=startContinuousCycle, args=(pill2kill,deviceId,relayId,delay) )
         t.setName("%s-%s" % (deviceId, relayId) )
         t.start()
         global threads
         threads[t.getName()] =  pill2kill
         print("Started continuous cycle %s" % t.getName() )

    def stopContinuousCycle(self,deviceId, relayId):
         threadName = "%s-%s" % (deviceId, relayId)
         if threadName in threads:
               pill2kill = threads[threadName]
               pill2kill.set()
               print "Stopped ", threadName
               threads.pop(threadName)

    def runOneCycle(self, deviceId, relayId, delay):
         threadName = "%s-%s" % (deviceId, relayId)
         if threadName in threads:
                print ("Duplicate one cycle for %s" % threadName)
                return

         print("Start one cycle")
         pill2kill = threading.Event()
         t = threading.Thread(target=startOneCycle, args=(pill2kill,deviceId,relayId,delay) )
         t.setName("%s-%s" % (deviceId, relayId) )
         t.start()
         global threads
         threads[t.getName()] =  pill2kill
         print("Started one cycle %s" % t.getName() )

    def echo(self, anything):
         print anything
         return "testing" #"Reply from RPC: %s" % anything
