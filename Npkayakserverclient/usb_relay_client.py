import zerorpc
import time

relay = {'horn':['U45Y4',1] , 
                    'sensor':['5SA7G',1],
                    'whiteLED':['5SA7G',2],
                    'greenLED':['X6SVH',1],
                    'redLED':['X6SVH',2] }


c = zerorpc.Client()
c.connect("tcp://127.0.0.1:4242")

def onDevice(rpcClient, device):
     if device in relay:
         rpcClient.openOneRelayChannel(relay[device][0],relay[device][1],)
     else:
         print("No such device: %s" % device )

def offDevice(rpcClient, device):
     if device in relay:
         rpcClient.closeOneRelayChannel(relay[device][0],relay[device][1],)
     else:
         print("No such device: %s" % device )

def offAllDevice(rpcClient):
      for device in relay.keys():
           offDevice(c, device)

def runContinuousCycle(rpcClient, device,delay):
     if device in relay:
         rpcClient.runContinuousCycle(relay[device][0], relay[device][1], delay)
     else:
         print("No such device: %s" % device )
     
def runOneCycle(rpcClient, device,delay):
     if device in relay:
         rpcClient.runOneCycle(relay[device][0], relay[device][1], delay)
     else:
         print("No such device: %s" % device )
     
def stopContinuousCycle(rpcClient, device):
     if device in relay:
         rpcClient.stopContinuousCycle(relay[device][0], relay[device][1])
     else:
         print("No such device: %s" % device )
     

try:
      
      offAllDevice(c);
      time.sleep(2)
      
      for device in relay.keys():
           onDevice(c, device)
           time.sleep(1)
           offDevice(c, device)

      onDevice(c,'greenLED')
      runContinuousCycle(c, 'whiteLED',  1)
      runContinuousCycle(c, 'redLED',  0.5)
      time.sleep(10)
      stopContinuousCycle(c,'whiteLED')
      runOneCycle(c, 'sensor', 15)
      time.sleep(15)
      stopContinuousCycle(c,'redLED')
      offDevice(c, 'greenLED')
      '''
      runContinuousCycle(c,'horn',1)
      time.sleep(2)
      stopContinuousCycle(c,'horn')
      '''
except Exception, e:
      print "An error occurred: %s" % e 